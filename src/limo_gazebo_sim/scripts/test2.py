...
class LineNavigator:
    ...
    def avoid_object(self):
        """Perform object avoidance maneuver."""
        rospy.loginfo("Avoiding object...")
        self.twist.linear.x = 0.1
        self.twist.angular.z = 0.5  # Rotate to steer away
        self.cmd_vel_pub.publish(self.twist)
        rospy.sleep(1.5)  # Duration for turning
        self.move_straight()  # Continue moving forward
        self.avoiding = False  # Reset avoidance flag
    ...
    def image_callback(self, msg):
        ...
        # Navigation Logic
        if not self.turning and not self.avoiding:
            if red_detected or green_detected:
                rospy.loginfo(f"Object detected: {'Red' if red_detected else 'Green'}. Avoiding.")
                self.avoiding = True
                self.avoid_object()
            elif self.detecting_orange and orange_detected:
